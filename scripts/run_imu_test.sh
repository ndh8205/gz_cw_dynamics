#!/usr/bin/env bash
# Headless OrbitImu verification.
set -eo pipefail
...
SCRIPT_INSTALLED="${HOME}/space_ros_ws/install/gz_cw_dynamics/lib/gz_cw_dynamics/test_imu.py"
SCRIPT="${SCRIPT_INSTALLED}"
[ -f "${SCRIPT}" ] || SCRIPT="${HOME}/space_ros_ws/src/gz_cw_dynamics/scripts/test_imu.py"
...
echo "[run_imu_test] world=${WORLD}"
gz sim -s -r --verbose 2 "${WORLD}" > /tmp/gz_imu_test.log 2>&1 &
GZ_PID=$!
cleanup() {
